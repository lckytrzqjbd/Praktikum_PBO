...
// ── Getter ─────────────────────────────────────────────────────
    ...
    public int getNilaiTugas1() {
        return nilaiTugas1;
    }

    public int getNilaiTugas2() {
        return nilaiTugas2;
    }
    ...
    public int getNilaiTugas3() {
        return nilaiTugas3;
    }

    public int getNilaiTugas4() {
        return nilaiTugas4;
    }
...
// ── Kalkulasi Nilai Akhir ──────────────────────────────────────
    ...
    /**
     * Menghitung nilai akhir berbobot. T1(10%) + T2(15%) + UTS(25%) + T3(10%) +
     * T4(15%) + UAS(25%)
     */
...
// ── QUERY: Ambil semua data (JOIN mahasiswa + nilai) ───────────
    ...
    public static ArrayList<MahasiswaModel> getAll() {
        ...
        String sql = "SELECT m.id, m.nim, m.nama, m.jurusan, "
                + " n.mata_kuliah, "
                + " n.nilai_tugas1, n.nilai_tugas2, n.nilai_UTS, "
...
String sqlNilai = "INSERT INTO nilai "
                + "(nim, mata_kuliah, nilai_tugas1, nilai_tugas2, nilai_UTS, "
                + " nilai_tugas3, nilai_tugas4, nilai_UAS) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = Koneksi.getConnection()) {
            conn.setAutoCommit(false); // gunakan transaksi
            try (PreparedStatement psMhs = conn.prepareStatement(sqlMhs); PreparedStatement psNilai = conn.prepareStatement(sqlNilai)) {
